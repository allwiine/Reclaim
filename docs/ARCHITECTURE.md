...
- **Typography — `Theme.TextRole`** (`TextRoles.swift`, applied via `.themeFont(_:)` from `ScaledFont.swift`). 47 named roles (`.rowTitle`, `.cardTitle`, `.headline`, `.tileValue`, `.warningIcon`, …), each pairing a fixed point size with a `Font.Weight`; views ask for a role, never a raw size. `.themeFont(_:)` and the ad hoc `.scaledFont(size:weight:design:)` both route their size through `@ScaledMetric(relativeTo: .body)`, so every screen renders pixel-identical to the static sizes at the system's default text size and grows only under larger Dynamic Type settings.
