...
Reclaim's visual language — color, spacing, typography, radii, motion — is centralized in `Sources/Reclaim/DesignSystem/`. Views describe screen structure; they never write down the styling literal itself. This isn't just convention: a set of `custom_rules` in `.swiftlint.yml` fails the lint (`error` severity, part of the pre-commit gate) on any raw `.padding(N)`, `spacing: N`, `cornerRadius: N`, `.font(.system(...))`/`Font.system(...)`, `scaledFont(size: N)`, or `Color(hex:)` written outside `Sources/Reclaim/DesignSystem/` — the one directory the rules exempt. **Only the DesignSystem states visual values**; everywhere else, a call site names a token.
...
- **Color** (`Theme.swift` for the core palette — surfaces, text, accent, semantic status colors, domain colors; `Palette+Views.swift` for view-local colors named while routing individual screens through the system). Every color is named for its role (`Theme.textRowLabel`, `Theme.calloutFill`, …), never referenced by hex value or opacity fill at the call site; `Color(hex:)` and `.opacity(...)`-composed fills are constructed exactly once, inside these two files.
