...
Views reach the model layer through the SwiftUI environment, not by holding `AppModel` directly: each declares `@Environment(<SubModel>.self)` for exactly the sub-models it reads, so its dependencies show up in its own property list instead of being implied by everything `AppModel` exposes. `@Environment(AppModel.self)` survives only where a view needs a cross-model aggregate that no single sub-model owns — `totalFoundBytes`, `cleanableBytes`, `selectedBytes`, `hasCleanableSelection`, `largestFindings(limit:)`, and `selectOnlySafe()` — each such use commented with the member it's kept for. `appEnvironment(_:)` (`Sources/Reclaim/App/AppEnvironment.swift`) is the single injection point, putting `AppModel` and all nine sub-models into the environment for both app scenes and SwiftUI previews. A `custom_rules` lint (`single_environment_root` in `.swiftlint.yml`) enforces that this is the *only* place `.environment(...)` is called with a model value, so a view can never route around the declared dependency list; keypath environment values (`.environment(\.locale, …)`) are exempt.

## DesignSystem

Reclaim's visual language — color, spacing, typography, radii, motion — is centralized in `Sources/Reclaim/DesignSystem/`. For the properties the gate covers, views never write down the styling literal itself: a set of `custom_rules` in `.swiftlint.yml` fails the lint (`error` severity, part of the pre-commit gate) on any raw `.padding(N)`, `spacing: N`, `cornerRadius: N`, `.font(.system(...))`/`Font.system(...)`, `scaledFont(size: N)`, or `Color(hex:)` written outside `Sources/Reclaim/DesignSystem/` — the one directory the rules exempt. That is a deliberately scoped v1: other layout literals — such as `.lineSpacing(N)`, `.frame(width:/height:)` sizes, `Spacer(minLength:)`, and `.tracking()` — stay inline at view call sites, unlinted. **Only the DesignSystem states visual values** for the properties above; everywhere else, a call site names a token.

- **Spacing — two tiers** (`Spacing.swift`). Tier 2, `Theme.Space`, is a 34-entry pixel ramp (`s0` … `s64`) — one entry per distance the UI actually uses, so any distance can go through a named token without inventing fake semantics. Tier 1 is a small set of semantic tokens named for their *role* rather than their value — `Theme.rowGap`, `Theme.chipPaddingHorizontal`/`chipPaddingVertical`, `Theme.radiusIconChip`, `Theme.radiusBadge` here, plus the older `Theme.cardPadding`, `Theme.cardGap`, `Theme.contentMargin`, and the `radiusHero`/`radiusCard`/`radiusTile`/`radiusPanel`/`radiusInset`/`radiusControl`/`radiusChip` ramp in `Theme.swift` — promoted out of the ramp once a real cluster of call sites shares a value. A token's value always equals the literal it replaced; re-theming is a deliberate, later act, not a side effect of adding a token.
- **Typography — `Theme.TextRole`** (`TextRoles.swift`, applied via `.themeFont(_:)` from `ScaledFont.swift`). 47 named roles (`.rowTitle`, `.cardTitle`, `.headline`, `.tileValue`, `.warningIcon`, …), each pairing a fixed point size with a `Font.Weight`; views ask for a role via `.themeFont(_:)`, never a raw size. The parameterized factories `Theme.mono(_:)` and `Theme.heroNumber(_:)` are the sanctioned escape for static fonts that do not scale with Dynamic Type. Both `.themeFont(_:)` and `.scaledFont(size:weight:design:)` route their size through `@ScaledMetric(relativeTo: .body)`, so every screen renders pixel-identical to the static sizes at the system's default text size and grows only under larger Dynamic Type settings.
- **Color** (`Theme.swift` for the core palette — surfaces, text, accent, semantic status colors, domain colors; `Palette+Views.swift` for view-local colors named while routing individual screens through the system; `Buttons.swift` and `Meters.swift` also construct a handful of `Color(hex:)` values directly — a strip-chip button's foreground and preview-only sample meter segments respectively). Every base color is named for its role (`Theme.textRowLabel`, `Theme.calloutFill`, …) rather than spelled out as a hex literal — `no_raw_hex_color` bans `Color(hex:)` anywhere outside `Sources/Reclaim/DesignSystem/`. Per-use emphasis opacity — a call site writing e.g. `Theme.caution.opacity(0.1)` to dim or tint an already-named color for one occasion (roughly twenty such sites across the views) — is deliberately out of scope for v1 and unlinted; only the base color value itself is enforced to come from the DesignSystem.

Adding a new value means adding a token in `DesignSystem` first, named by role; if no existing semantic cluster fits, a sweep drops it into `Theme.Space`'s ramp at the matching value rather than inventing a one-off semantic name. See [CONTRIBUTING.md](../CONTRIBUTING.md)'s Tooling section for the enforcing lint rules themselves.
